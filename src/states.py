...
class InitialState(AppState):
    ...
    def register(self):
        ...
        self.register_transition(States.WEB_CONTROLLED, role=Role.PARTICIPANT)
    ...
    def run(self):
        ...
        # if no input files are in the input directory, the app is web controlled
        if len(os.listdir(INPUT_DIR)) == 0:
            self.log("Transitioning to WEB_CONTROLLED state")
            self.store('is_web_controlled', True)
